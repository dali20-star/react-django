...
class TestResultViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        """Set the uploaded_by field to current authenticated user"""
        serializer.save(uploaded_by=self.request.user)
    
    def perform_update(self, serializer):
        """Update while preserving the original uploaded_by"""
        serializer.save()
    ...
    def statistics(self, request):
        ...
        passed_tests = queryset.filter(status__in=['PASSED', 'PASS']).count()
        failed_tests = queryset.filter(status__in=['FAILED', 'FAIL']).count()
        ...
        recent_tests = queryset.order_by('-executed_at')[:5]
        recent_tests_data = []
